<template>
  <div>
    <div v-if="subjectInfo">
      <div class="table-condition-btnbar" style="padding: 0;margin-bottom:0;">
        <div class="manage-content-breadcrumb">
          <span class="manage-content-breadcrumb-span" style="cursor:pointer;" @click="subjectInfo = null">全部话题</span>
        ...
        <div class="discuss-subject-main">
          ...
          <div class="discuss-subject-content">
            <div class="discuss-content-img">
              <img v-if="subjectInfo.userImage" class="header-adavor-img" :src="subjectInfo.userImage" />
              <img v-else class="header-adavor-img" src="static/images/noperson.png" />
            </div>
            <div class="discuss-content-userdiv">
              {{subjectInfo.userName}}
              <span style="margin-left:0;">{{convertUser(subjectInfo.userType)}}</span>
        ...
        <div class="detail-main">
          <div class="discuss-reply-main">
            <div class="discuss-reply-list">
              <div class="discuss-reply-list-item" :key="item" v-for="(item,index) in data2">
                ...
                <div class="discuss-content-userdiv">
                  ...
                  <span v-if="!isForbid && (userInfo.id != item.userId)" @click="replyTarget = item"
                    class="replythis">回复此人</span>
                </div>
                <div class="discuss-content-textdiv">
                  <div v-if="item.replayId" class="discuss-content-replytargetdiv">
                    <p style="border-bottom: 1px dotted #ccc;">
                      ...
                      {{item.replyUserName}}
                      <span v-if="item.replyTarget" style="float:right;">{{item.replyTarget.floorNumber}}楼</span>
                    </p>
                    <p v-if="item.replyTarget" class="text-ellipsis">{{item.replyTarget.content}}</p>
                    <p v-else @click="getTarget(item,index)" class="text-ellipsis getTarget">--- 查看内容 ---</p>
                  </div>
                  {{item.content}}
                </div>
              </div>
            ...
            <div v-if="data2.length>0" class="discuss-reply-page">
              <Page :total="pageData1.total" :current="pageData1.current" :page-size="pageData1.pageSize"
                prev-text="上一页" next-text="下一页" placement="top" @on-change="currentChange" />
            </div>
            <div class="discuss-reply-content">
              ...
              <div v-if="replyTarget" class="discuss-content-userdiv" style="border:none;margin-right: 20px;">
                回复：
                <div class="discuss-content-replytargetdiv">
                  <p style="border-bottom: 1px dotted #ccc;">
                    {{replyTarget.userName}}
                    <span style="margin-left:0;">{{convertUser(replyTarget.userType)}}</span>
    ...
    <div v-else>
      ...
      <b-table @page-data-change="currentChange1" @page-size-change="sizeChange" :pageData="pageData"
        @on-row-click="rowClick" :columns="columns1" :dataSource="data1"></b-table>
    </div>

    <Drawer title="话题" v-model="showAddNew" width="720" :mask-closable="false" class-name="demo-drawer-style">
