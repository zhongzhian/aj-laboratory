    ...
    <div class="manage-content-right-content">
      ...
      <div v-if="sectionTitle">
        <div class="normal-tabs">
          <div style="position:absolute;top:5px;left:400px;width:100px;">
            <Button
              v-if="sectionInfo.studyStatus === 2"
              @click="endStudy"
              size="small"
              class="condition-btn"
              >完成学习</Button
  ...
  methods: {
      ...
      this.axios
        ...
        .then(result => {
          if (result.code === 20000) {
            ...
            this.sectionInfo.studyStatus = 2;
            section.studyStatus = 2;
    ...
    selectSections(section) {
      ...
      this.axios.get(`${API.index.section}/` + this.sectionId).then(result => {
        if (result.code === 20000) {
          this.sectionInfo = result.result;
          this.sectionInfo.studyStatus = section.studyStatus;
                ...
                r => {
                  if (
                    ...
                    r.sectionStudyRecordList = r.sectionStudyRecordList.map(
                      s => {
                        // 0-未学习 1-已学习 2-学习中
                        let studyStatusArr = ["未学习", "已学习", "学习中"];
                        let studiedStr = studyStatusArr[s.studyStatus];
                        return {
                          ...s,
