    ...
    <div v-if="myDevice" class="lab-device-div">
      ...
      <div v-if="isStart" class="lab-device-div-content">
        <div v-if="isDevice" class="iframe-close-div">
          <Icon v-show="isShowVideo" class="iframe-close-icon" @click.stop="hiddenVideo" type="ios-pause" />
          <Icon v-show="!isShowVideo" class="iframe-close-icon" @click.stop="showVideo" type="ios-play" />
        </div>
        <iframe :src="myDeviceDetail.pageUrl" width="100%" height="100%" style="background: #f1f2f3;"
          frameborder="0"></iframe>

        <div v-show="(isDevice&&isShowVideo)" class="lab-device-video-div" @mousedown="move"
          :style="'left:'+positionX+'px;top:'+positionY+'px;'">
          <div class="lab-device-video-title">
            实验视频
            <Icon style="color:#fff;float:right;margin-top: 10px;cursor:pointer;" @click.stop="hiddenVideo"
              type="close" />
          </div>
          <div class="lab-device-video-content">
            <video id="example_video_1" width="100%" height="300" name="videoElement" class="centeredVideo" controls
              autoplay muted>Your browser is too old which doesn't support HTML5 video.</video>
          </div>
        </div>
      </div>
    </div>
