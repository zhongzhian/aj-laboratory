<template>
  <div>
    <div class="table-condition-btnbar">
      <Input v-model="searchName" size="small" placeholder="名称/设备号" style="width:200px;" />
    ...
    <div>
      ...
      <b-table @on-select="rowSelect" @page-data-change="currentChange" :pageData="pageData1" :columns="columns1"
        :dataSource="data1"></b-table>
    </div>

    <Drawer title="设备" v-model="showAddNew" width="720" :mask-closable="false" class-name="demo-drawer-style">
      <div class="demo-drawer-content">
        <Form ref="formValidate" :model="newObj" :rules="ruleValidate">
          ...
          <Row :gutter="32">
            ...
            <Col span="12">
            <FormItem label="设备号" prop="resourceClass" label-position="top">
              <Input v-model="newObj.resourceClass" size="small" placeholder="请输入设备号" />
            </FormItem>
            </Col>
          </Row>
          <Row :gutter="32">
            ...
            <FormItem label="关联的摄像头" label-position="top">
              <Select v-model="cameraId" placeholder="请选择关联的摄像头">
                <Option v-for="item in cameraDatas" :value="item.value" :key="item">{{ item.label }}</Option>
              </Select>
            </FormItem>
            </Col>
            <Col span="12">
    ...
    <Drawer title="信号通道列表" v-model="showChannel" width="720" :mask-closable="false" class-name="demo-drawer-style">
      <div class="demo-drawer-content">
        <div class="table-condition-btnbar">
        ...
        <Table size="small" :columns="columns2" :data="channelDatas" @on-row-click="channelrowClick"
          @on-select="channelrowSelect"></Table>
      </div>
      <div class="demo-drawer-footer">
    ...
    <Drawer title="信号通道" v-model="showEditChannel" width="600" :mask-closable="false" class-name="demo-drawer-style">
      <div class="demo-drawer-content">
        <Form ref="channelValidate" :model="channelObj" :rules="ruleValidate">
          <FormItem label="图片" prop="picture">
            <Upload ref="upload" :action="uploadAction" :headers="headers" :data="uploadData" :show-upload-list="false"
              :on-success="handleSuccess2" :format="['jpg', 'jpeg', 'png']" :max-size="2048"
              :on-exceeded-size="handleMaxSize" :before-upload="handleBeforeUpload"
              style="display: inline-block;vertical-align: top;">
              <img v-if="channelObj.picture" :src="channelObj.picture" class="form-user-img" />
              <img v-if="!channelObj.picture" src="static/images/noperson.png" class="form-user-img" />
            </Upload>
          </FormItem>
          ...
          <FormItem label="数据类型" prop="dataType">
            ...
            <Select v-model="channelObj.dataType" placeholder="请输入数据类型">
              <Option v-for="item in channelTypes" :value="item.value" :key="item">{{ item.label }}</Option>
            </Select>
          </FormItem>
          <FormItem label="描述" prop="description" label-position="top">
